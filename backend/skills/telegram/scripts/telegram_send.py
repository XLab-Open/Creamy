...
def main():
    ...
    parser.add_argument("--token", "-t", help="Bot token (defaults to CREAMY_TELEGRAM_TOKEN env var)")
    ...
    # Get bot token
    bot_token = args.token or os.environ.get("CREAMY_TELEGRAM_TOKEN")
    if not bot_token:
        print("❌ Error: Bot token required. Set CREAMY_TELEGRAM_TOKEN env var or use --token")
        sys.exit(1)
