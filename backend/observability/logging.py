...
optional logfire APM. Tune it via ``CREAMY_LOG_*`` env vars (see docs/logging.md).
...
def setup_logging(*, force: bool = False) -> None:
    ...
    With no ``CREAMY_LOG_*`` overrides the console behaves as before, plus a
    ``~/.creamy/logs/creamy.log`` file sink is added.
    """
    global _CONFIGURED
    ...
    diagnose = env("CREAMY_LOG_DIAGNOSE", "0") == "1"  # off in prod: keeps variable values/secrets out of tracebacks
    ...
    logger.add(
        log_dir / "creamy.log",
        level=file_level,
        rotation=env("CREAMY_LOG_ROTATION", "20 MB"),
        retention=env("CREAMY_LOG_RETENTION", "14 days"),
        compression="zip",
        enqueue=True,
        backtrace=True,
        diagnose=diagnose,
