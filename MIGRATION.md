...
- **Codex OAuth 端到端**：离线只验证了「客户端装配正确」；真实 OAuth 握手 + ChatGPT backend 响应需一次真实 `creamy login openai` + 网络。
...
- `CreamyFramework` / `HookRuntime` / 16 个 `CreamyHookSpecs` 的名称、数量、语义、调用顺序**全部保留**。
