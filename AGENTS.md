...
- `backend/__main__.py` — Typer CLI entrypoint (`creamy = "backend.__main__:app"`).
...
- `backend/skills/` — skills bundled with Creamy.
...
- `uv run creamy chat` — interactive CLI.
- `uv run creamy gateway` — start channel listeners.
- `uv run creamy run "hello"` — run a single message through the full pipeline.
...
- Runtime settings come from `CREAMY_*` variables — e.g. `CREAMY_MODEL`, `CREAMY_API_KEY`,
  `CREAMY_API_BASE`. See `env.example` for the full list.
- Provider-specific keys (e.g. `OPENROUTER_API_KEY`) may still be read by downstream SDKs.
- Telegram needs `CREAMY_TELEGRAM_TOKEN`; restrict access with `CREAMY_TELEGRAM_ALLOW_USERS`
  and `CREAMY_TELEGRAM_ALLOW_CHATS`.
