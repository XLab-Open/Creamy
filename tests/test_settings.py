...
def test_settings_single_api_key_and_base() -> None:
    settings = _settings_with_env({"CREAMY_API_KEY": "sk-test", "CREAMY_API_BASE": "https://api.example.com"})
...
def test_settings_per_provider_keys() -> None:
    settings = _settings_with_env({
        "CREAMY_OPENAI_API_KEY": "sk-openai",
        "CREAMY_OPENAI_API_BASE": "https://api.openai.com",
        "CREAMY_ANTHROPIC_API_KEY": "sk-anthropic",
    })
...
def test_settings_provider_names_are_lowercased() -> None:
    settings = _settings_with_env({"CREAMY_OPENROUTER_API_KEY": "sk-or"})
...
""".strip(),
    ...
    with patch.dict("os.environ", {"CREAMY_HOME": str(tmp_path)}, clear=True):
...
""".strip(),
    ...
    with patch.dict(
        "os.environ",
        {
            "CREAMY_HOME": str(tmp_path),
            "CREAMY_MODEL": "anthropic:claude-3-7-sonnet",
            "CREAMY_API_KEY": "sk-env",
            "CREAMY_CLIENT_ARGS": '{"extra_headers":{"HTTP-Referer":"https://env.example","X-Title":"Env App"}}',
            "CREAMY_MAX_STEPS": "12",
        },
        clear=True,
    ):
...
def test_settings_client_args_can_be_disabled() -> None:
    settings = _settings_with_env({"CREAMY_CLIENT_ARGS": "null"})
